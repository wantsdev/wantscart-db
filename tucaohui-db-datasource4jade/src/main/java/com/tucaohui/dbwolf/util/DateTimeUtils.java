...
public class DateTimeUtils {
    ...
    public abstract class Value {
        ...
        public static final int NULL = 0, BOOLEAN = 1, BYTE = 2, SHORT = 3, INT = 4, LONG = 5,
                DECIMAL = 6;

        public static final int DOUBLE = 7, FLOAT = 8, TIME = 9, DATE = 10, TIMESTAMP = 11,
                BYTES = 12;
    ...
    public static java.util.Date parseDateTime(String original, int type, int errorCode)
        ...
        try {
            ...
            long time;
            try {
                time = getTime(false, tz, year, month, day, hour, minute, second,
                        type != Value.TIMESTAMP, nano);
            } catch (IllegalArgumentException e) {
                // special case: if the time simply doesn't exist, use the lenient version
                if (e.toString().indexOf("HOUR_OF_DAY") > 0) {
                    time = getTime(true, tz, year, month, day, hour, minute, second,
                            type != Value.TIMESTAMP, nano);
                } else {
                    throw e;
                }
    ...
    private static long getTime(boolean lenient, TimeZone tz, int year, int month, int day,
            int hour, int minute, int second, boolean setMillis, int nano) {
        Calendar c;
        if (tz == null) {
