 ...
 */
package com.tucaohui.dbwolf.util;
...
public class Queue<T> {
    ...
    /**
     * Appends an item to the queue and notify a listener regardless of how
     * many items are on the queue. Use this for the last item you append
     * to a queue in a batch via <code>appendSilent</code> because the
     * regular <code>append</code> will think it doesn't need to notify
     * anyone because the queue size isn't zero prior to this add. You
     * should also use this method if you have mutiple consumers listening
     * waiting on the queue, to guarantee that one will be woken for every
     * element added.
     */
    ...
    /**
     * Internal append method. If subclassing queue, be sure to call this
     * method from inside a synchronized block.
     */
     ...
     * This method will not block waiting for an item to be added to the
     * queue.
     */
    ...
    public synchronized T get() {
        ...
        if ((_size > MIN_SHRINK_SIZE) && (_size > _suggestedSize) && (_count < (_size >> 3))) shrink();
