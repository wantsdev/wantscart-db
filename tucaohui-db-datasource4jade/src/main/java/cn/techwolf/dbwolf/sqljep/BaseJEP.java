...
package cn.techwolf.dbwolf.sqljep;
...
import cn.techwolf.dbwolf.sqljep.function.Comparative;
import cn.techwolf.dbwolf.sqljep.function.ComparativeBaseList;
import cn.techwolf.dbwolf.sqljep.function.PostfixCommand;
import cn.techwolf.dbwolf.sqljep.function.PostfixCommandI;
import cn.techwolf.dbwolf.sqljep.variable.Variable;
import cn.techwolf.dbwolf.util.StaticString;
import cn.techwolf.dbwolf.util.ThreadLocalMap;

/**
 * Base class for different SQLJEP classes. This class doesn't know how to
 * get source data.
 * <p/>
 ...
 * {@link #getColumnObject(int column)} for defining access to abstarct
 * table data and the method {@link #getVariable(String name)} to access to
 * external variables.
 * <p/>
 * To get a variable value at first {@link #getVariable(String name)} and
 * after that {@link #findColumn(String name)} method is used.
 * <p/>
 * In expressions 'part_1.part2' names of variables are supported. Parser
 * is case sensitive.
 * <p/>
 * Visit <a
...
public abstract class BaseJEP implements ParserVisitor {
     ...
     * <p/>
     * Sometimes it is necessary to change value of a SQLJEP expression by
     * some external reasons. Then following code can be applied
     * <blockquote>
     * 
     * <pre>
     ...
     * @throws cn.techwolf.dbwolf.sqljep.ParseException
     */
    ...
    /**
     * Parses the expression. The root of the expression tree is returned
     * by {@link #getTopNode()} method
     * 
     * @throws cn.techwolf.dbwolf.sqljep.ParseException If there are errors
     *         in the expression then it fires the exception
     */
    final public void parseExpression(Map<String, Integer> columnMapping,
            Map<String, Variable> variableMapping, final Map<String, PostfixCommand> funMap)
        ...
        try {
            ...
            throw new cn.techwolf.dbwolf.sqljep.ParseException(toString(), e);
        }

        if (hasError()) {
            throw new cn.techwolf.dbwolf.sqljep.ParseException(getErrorInfo());
        }
     ...
     * {@link cn.techwolf.dbwolf.sqljep.JepRuntime#stack} for evaluation.
     * <p>
    ...
    /**
     * Compares two objects for equality. Two SQLJEPs are equals only when
     * their String representations are equals.
     */
    ...
    /**
     * Visit a function node. The values of the child nodes are first
     * pushed onto the stack. Then the function class associated with the
     * node is used to evaluate the function.
     */
    ...
    /**
     * Visit a variable node. The value of the variable is obtained from
     * the model and pushed onto the stack.
     */
    @SuppressWarnings("unchecked")
